...
class FrameMatcher:
    ...
    def process(self, df_det, df_rep):
        ...
        alpha = df_det['confidence'].to_numpy()
        q = comm_quality(df_rep['snr'], df_rep['rssi'], df_rep['delay'], df_rep['coverage'])
...
if __name__ == "__main__":
    ...
    df_det = pd.read_csv(cfg.det_csv_path)
    df_rep = pd.read_csv(cfg.rep_csv_path)

    times = sorted(set(df_det['time']).intersection(set(df_rep['time'])))
    all_res = []

    for t in times:
        f_det = df_det[df_det['time']==t]
        f_rep = df_rep[df_rep['time']==t]
        res = matcher.process(f_det, f_rep)
        if cfg.print_detail:
            print(f"\n⏱ Time {t}")
            print("Matches:", res["matches"])
            print("Unreported:", res["unreported"])
            print("Undetected:", res["undetected"])
            print("AbnormalQueue:", res["AbnormalQueue"])
        all_res.append(res)

    print("\n✅ 全部帧处理完毕，总异常目标：", matcher.AbnormalQueue)
